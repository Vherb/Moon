...
function attachHandlers(){
        ...
        if(isOpen(opp)) {
          ...
          try {
            ...
            if (oppPos && Number.isFinite(oppPos.x) && Number.isFinite(oppPos.z)) {
              ...
              payload.isJetpacking = !!oppPos.isJetpacking;
              payload.isShooting = !!oppPos.isShooting;
              payload.isAiming = !!oppPos.isAiming;
              payload.isWalkingBackward = !!oppPos.isWalkingBackward;
              payload.isStrafeLeft = !!oppPos.isStrafeLeft;
              payload.isStrafeRight = !!oppPos.isStrafeRight;
              payload.isDead = !!oppPos.isDead;
              if (typeof oppPos.tiltX === 'number') payload.tiltX = oppPos.tiltX;
              if (typeof oppPos.tiltZ === 'number') payload.tiltZ = oppPos.tiltZ;
              if (typeof oppPos.pitch === 'number') payload.pitch = oppPos.pitch;
  ...
  case 'avatarMove': {
        ...
        try {
          ...
          // basic throttle per socket to avoid flooding
          const now = Date.now();
          st1._lastAvatarTs = st1._lastAvatarTs || 0;
          const isFlying = !!data.isJetpacking;
          const minInterval = isFlying ? 16 : 35;
          if ((now - st1._lastAvatarTs) < minInterval) { break; }
          ...
          const isJetpacking = !!data.isJetpacking;
          const isShooting = !!data.isShooting;
          const isAiming = !!data.isAiming;
          const isWalkingBackward = !!data.isWalkingBackward;
          const isStrafeLeft = !!data.isStrafeLeft;
          const isStrafeRight = !!data.isStrafeRight;
          const isDead = !!data.isDead;
          const tiltX = (typeof data.tiltX === 'number') ? data.tiltX : 0;
          const tiltZ = (typeof data.tiltZ === 'number') ? data.tiltZ : 0;
          ...
          room.avatarPositions[side] = { x, z, yaw, run, isJumping, isJetpacking, isShooting, isAiming, isWalkingBackward, isStrafeLeft, isStrafeRight, isDead, tiltX, tiltZ, pitch, lift };
          ...
          payload.isJetpacking = !!isJetpacking;
          payload.isShooting = !!isShooting;
          payload.isAiming = !!isAiming;
          payload.isWalkingBackward = !!isWalkingBackward;
          payload.isStrafeLeft = !!isStrafeLeft;
          payload.isStrafeRight = !!isStrafeRight;
          payload.isDead = !!isDead;
